    ...
    <div style={{paddingBottom:16}}>
      ...
      <div style={{background:"#0D1117",border:"1px solid rgba(255,255,255,.07)",borderRadius:18,overflow:"hidden",marginBottom:14}}>
        ...
        <div style={{padding:"16px"}}>
          ...
          {/* Aperçu pari */}
          {form.team&&form.odds&&(
            <div style={{padding:"11px 14px",background:"rgba(124,58,237,.08)",border:"1px solid rgba(124,58,237,.18)",borderRadius:12,marginBottom:14,display:"flex",alignItems:"center",gap:10}}>
              {teamLogo&&<img src={teamLogo} alt="" style={{width:24,height:24,objectFit:"contain"}}/>}
              <div style={{fontSize:12,color:"#c4b5fd",fontWeight:600,flex:1}}>{getBetDescription()}{form.opponent?" vs "+form.opponent:""}</div>
              <div style={{fontSize:13,fontWeight:800,color:"#a78bfa"}}>@{form.odds}</div>
              {form.stake&&<div style={{fontSize:12,fontWeight:700,color:"#E5E7EB"}}>{form.stake}$</div>}
            </div>
          )}
          ...
          <div key={b.id} style={{background:"#0D1117",border:"1px solid rgba(255,255,255,.07)",borderLeft:"3px solid "+col+(isPending?"":"AA"),borderRadius:14,marginBottom:10,overflow:"hidden"}}>
            <div style={{padding:"13px 14px"}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
                {bLogo
                  ?<img src={bLogo} alt={b.team} style={{width:34,height:34,objectFit:"contain",flexShrink:0}}/>
                  :<div style={{width:34,height:34,borderRadius:8,background:"rgba(255,255,255,.04)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><span style={{fontSize:18}}>🏀</span></div>}
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:1}}>
                    <div style={{fontWeight:800,fontSize:14,color:"#E5E7EB",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{b.description}</div>
                    {b.betType==="3pts"&&<span style={{fontSize:9,fontWeight:800,color:"#34d399",background:"rgba(52,211,153,.1)",border:"1px solid rgba(52,211,153,.25)",borderRadius:5,padding:"1px 5px",flexShrink:0}}>3PT</span>}
                    {b.betType==="handicap"&&<span style={{fontSize:9,fontWeight:800,color:"#a78bfa",background:"rgba(124,58,237,.1)",border:"1px solid rgba(124,58,237,.25)",borderRadius:5,padding:"1px 5px",flexShrink:0}}>HCP</span>}
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:5,marginTop:2}}>
                    <GameLogo game={b.league} size={12}/>
                    <span style={{fontSize:11,color:"#6B7280"}}>{b.league}{b.opponent?" · vs "+b.opponent:""}</span>
                  </div>
                </div>
                <div style={{textAlign:"right",flexShrink:0}}>
                  ...
                  <div style={{fontSize:10,color:"#6B7280",marginTop:1}}>{b.bookmaker}</div>
                </div>
              </div>
              <div style={{fontSize:11,color:"#4B5563",marginBottom:isPending?10:4}}>@{b.odds} · {b.stake}$ · {b.datetime?b.datetime.slice(0,10):""}</div>
          ...
          <div className="view-enter" style={{margin:"-16px -14px",padding:"0",minHeight:"calc(100vh - 84px)",background:"radial-gradient(circle at 50% -12%,rgba(42,73,145,.35),transparent 35%),linear-gradient(180deg,#080e1c,#050916 72%)"}}>
            ...
            {!duelMode&&<div style={{background:"linear-gradient(180deg,rgba(14,20,38,.98),rgba(8,12,24,.99))",borderRadius:18,border:"1px solid rgba(139,92,246,.2)",padding:"11px 12px 12px",marginBottom:8,boxShadow:"0 8px 24px rgba(0,0,0,.2)"}}>
              ...
              <input type="datetime-local" value={form.datetime||nowDT()} onChange={e=>setForm(f=>({...f,datetime:e.target.value}))}
                style={{width:"100%",background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.1)",borderRadius:12,padding:"11px 14px",color:"#E5E7EB",fontSize:13,fontFamily:"Inter,sans-serif",outline:"none",boxSizing:"border-box",colorScheme:"dark"}}/>
            </div>}
              ...
              return(
                <div style={{background:"#080e1e",borderRadius:20,border:"1px solid rgba(255,255,255,.08)",overflow:"hidden",marginBottom:10}}>
                  <div style={{padding:"12px 16px",borderBottom:"1px solid rgba(255,255,255,.05)",display:"flex",alignItems:"center",gap:10}}>
                    {tbLogo&&<img src={tbLogo} alt={tbTeam} style={{width:22,height:22,objectFit:"contain"}} loading="lazy"/>}
                    <div style={{fontSize:11,fontWeight:700,color:"#6B7280",textTransform:"uppercase",letterSpacing:1.2,flex:1}}>{tbTeam||"Pari Équipe"}</div>
                    <button onClick={()=>{setTeamBetMode(false);setForm(f=>({...f,tbTeam:"",player:"",game:"NBA"}));}}
                      style={{background:"none",border:"none",color:"#4B5563",fontSize:12,cursor:"pointer",fontFamily:"Inter,sans-serif",padding:"2px 6px"}}>Changer ×</button>
                  </div>
                  <div style={{padding:"14px 16px",display:"flex",flexDirection:"column",gap:14}}>
                    {/* Barre recherche équipe toujours visible */}
                    <div style={{position:"relative"}}>
                      ...
                      }} style={{width:"100%",background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.1)",borderRadius:12,padding:"12px 40px 12px 14px",color:tbTeam?"#E5E7EB":"#4B5563",fontSize:13,fontFamily:"Inter,sans-serif",outline:"none",cursor:"pointer",appearance:"none",WebkitAppearance:"none",fontWeight:tbTeam?600:400}}>
                        <option value="">Choisir une équipe…</option>
                    ...
                    {tbTeam&&(
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                        <div>
                          ...
                          <input type="text" inputMode="decimal" placeholder="1.85 ou 185" value={form.odds||""} onChange={e=>{
                    ...
                    {savedTipsters.length>0&&(
                      <div>
                        <div style={{fontSize:10,color:"#4B5563",fontWeight:700,textTransform:"uppercase",letterSpacing:.8,marginBottom:6}}>Tipster</div>
                        <div style={{position:"relative"}}>
                          <select value={tipsterName} onChange={e=>setTipsterName(e.target.value)}
                            style={{width:"100%",background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.1)",borderRadius:12,padding:"11px 36px 11px 12px",color:tipsterName?"#E5E7EB":"#6B7280",fontSize:13,fontFamily:"Inter,sans-serif",outline:"none",appearance:"none",WebkitAppearance:"none",cursor:"pointer"}}>
                    ...
                    {/* Mise rapide */}
                    {form.stake&&<div style={{fontSize:10,color:"#4B5563",marginTop:-8}}>
                      <span style={{color:"#4ade80",fontWeight:600}}>Gain potentiel : +{gain}$</span>
                    </div>}
                    {!form.stake&&<div>
                      <div style={{fontSize:10,color:"#4B5563",fontWeight:700,textTransform:"uppercase",letterSpacing:.8,marginBottom:6}}>Mise rapide</div>
                      <div style={{display:"flex",gap:6}}>
                        {[25,50,100].map(v=>(
                          <button key={v} onClick={()=>setForm(f=>({...f,stake:String(v)}))}
                            style={{flex:1,padding:"9px 0",borderRadius:10,border:"1px solid rgba(255,255,255,.08)",background:"rgba(255,255,255,.03)",color:"#9CA3AF",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"Inter,sans-serif"}}>
                            {v}$
                          </button>
                        ))}
                      </div>
                    ...
                    {/* Bouton Combiner */}
                        <button onClick={()=>{
                          ...
                          // Pré-remplir le combiné avec la sélection équipe actuelle
                          ...
                          style={{width:"100%",height:44,border:"1.5px dashed rgba(52,211,153,.3)",borderRadius:13,background:"rgba(52,211,153,.04)",color:"#34d399",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"Inter,sans-serif",marginBottom:8,display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
                  ...
                  {isOpen&&(
                    <div style={{background:"#111827",border:"1px solid #1F2937",borderTop:"none",borderRadius:"0 0 14px 14px",overflow:"hidden"}}>
                      ...
                      {gs.tourneys.length>0&&(()=>{
                        const cups=gs.tourneys.filter(t=>t.isCup);
                        const others=gs.tourneys.filter(t=>!t.isCup);
                        return(<>
                          {cups.length>0&&(
                            <><div style={{fontSize:11,color:"#FCD34D",fontWeight:800,letterSpacing:1.5,textTransform:"uppercase",padding:"14px 14px 6px",borderBottom:"1px solid rgba(251,191,36,0.2)",fontFamily:"'Inter',sans-serif",borderTop:"1px solid #1F2937"}}>🏆 Coupes</div>
                              {cups.map(t=>{
                                ...
                                return(
                                  <div key={t.name} className="stat-row" onClick={()=>setStatsDrill({game,league:null,filterType:"tourney",filterValue:t.name})} style={{cursor:"pointer"}}>
                                    <div style={{display:"flex",alignItems:"center",gap:7}}>
                                      <div style={{width:18,height:18,borderRadius:4,background:"rgba(251,191,36,0.1)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                                        {cupObj&&cupObj.logo?<img src={cupObj.logo} alt="" style={{width:16,height:16,objectFit:"contain"}} onError={e=>e.target.style.display="none"}/>:<span style={{fontSize:12}}>🏆</span>}
                                      </div>
                                      <div><div style={{display:"flex",alignItems:"center",gap:5}}><div style={{fontWeight:600,fontSize:13,color:"#FCD34D"}}>{cupRealName}</div><span style={{fontSize:10,color:"#4B5563"}}>›</span></div><div style={{fontSize:10,color:"#6B7280"}}>{t.count} paris · {wr.toFixed(0)}% WR</div></div>
                                    </div>
                                    <div style={{textAlign:"right"}}><div style={{fontWeight:700,fontSize:13,color:t.profit>=0?"#22C55E":"#EF4444"}}>{t.profit>=0?"+":""}{(t.profit||0).toFixed(0)}$</div><div style={{fontSize:10,color:roi>=0?"#22C55E":"#EF4444"}}>{roi>=0?"+":""}{roi.toFixed(1)}%</div></div>
                                  </div>
                                );
                              })}
                            </>
                              ...
                              {others.map(t=>{
                                ...
                                return(
                                  <div key={t.name} className="stat-row" onClick={()=>setStatsDrill({game,league:null,filterType:"tourney",filterValue:t.name})} style={{cursor:"pointer"}}><div style={{display:"flex",alignItems:"center",gap:7}}>
                                      {t.name==="Hors tournoi"?<span style={{fontSize:12}}>📅</span>:<LeagueLogo league={t.name} size={18}/>}
